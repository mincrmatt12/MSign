...
bool weather::loop() {
	...
	serial::interface.update_slot(slots::WEATHER_INFO, info);
	serial::interface.update_slot(slots::WEATHER_ARRAY, state_data, sizeof state_data);
	serial::interface.update_slot(slots::WEATHER_TEMP_GRAPH, temp_over_day, sizeof temp_over_day);
	serial::interface.update_slot(slots::WEATHER_TIME_SUN, suntimes);
